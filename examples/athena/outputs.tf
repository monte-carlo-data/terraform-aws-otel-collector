output "grpc_endpoint" {
  description = "The gRPC endpoint for the OpenTelemetry Collector"
  value       = module.otel_collector.opentelemetry_collector_grpc_endpoint
}

output "http_endpoint" {
  description = "The HTTP endpoint for the OpenTelemetry Collector"
  value       = module.otel_collector.opentelemetry_collector_http_endpoint
}

output "external_access_role_arn" {
  description = "The ARN of the IAM role for external access to the OpenTelemetry S3 bucket"
  value       = module.otel_collector.opentelemetry_collector_external_access_role_arn
}

output "external_access_role_name" {
  description = "The name of the IAM role for external access to the OpenTelemetry S3 bucket"
  value       = module.otel_collector.opentelemetry_collector_external_access_role_name
}

output "security_group_id" {
  description = "The ID of the security group for the OpenTelemetry Collector"
  value       = module.otel_collector.opentelemetry_collector_security_group_id
}

# Athena/Glue Resources Outputs
output "athena_glue_classifier_name" {
  description = "The name of the Glue classifier for parsing telemetry data"
  value       = module.otel_collector.athena_glue_classifier_name
}

output "athena_sns_topic_arn" {
  description = "The ARN of the SNS topic used for notifications (either provided or created by the module)"
  value       = module.otel_collector.athena_sns_topic_arn
}

output "athena_sqs_queue_arn" {
  description = "The ARN of the SQS queue that receives notifications from SNS and triggers the Glue crawler"
  value       = module.otel_collector.athena_sqs_queue_arn
}

output "athena_sqs_queue_url" {
  description = "The URL of the SQS queue for the Glue crawler"
  value       = module.otel_collector.athena_sqs_queue_url
}

output "athena_glue_crawler_role_arn" {
  description = "The ARN of the IAM role used by the Glue crawler"
  value       = module.otel_collector.athena_glue_crawler_role_arn
}

output "athena_glue_crawler_role_name" {
  description = "The name of the IAM role used by the Glue crawler"
  value       = module.otel_collector.athena_glue_crawler_role_name
}

output "athena_glue_crawler_name" {
  description = "The name of the Glue crawler that processes telemetry data"
  value       = module.otel_collector.athena_glue_crawler_name
}

output "athena_glue_database_name" {
  description = "The name of the Glue catalog database containing telemetry tables"
  value       = module.otel_collector.athena_glue_database_name
}

